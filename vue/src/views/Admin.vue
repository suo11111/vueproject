<template>
  <div>
    ...
    <el-dialog title="店长信息" v-model="data.formVisible" width="500" destroy-on-close>
...
const add=()=>{
  request.post("/admin/add",data.form).then(res=>{//新增没有id
    if(res.code === '200'){
...
const update=()=>{
  request.put("/admin/update",data.form).then(res=>{//修改有id
    if(res.code === '200'){
      data.formVisible = false
      ElMessage.success('操作成功')
      load()//重新加载数据
